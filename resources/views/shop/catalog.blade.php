        ...
        <aside class="lg:col-span-3 mb-8 lg:mb-0 lg:sticky lg:top-24">
            <div class="space-y-5">
                <form method="GET" class="relative">
                    ...
                    <button type="submit" aria-label="Search" class="absolute right-1 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-brand text-white grid place-items-center">
                ...
                <div class="rounded-2xl bg-white border border-[var(--line)] overflow-hidden">
                    <button type="button" class="cat-toggle w-full flex items-center justify-between px-4 py-3 text-sm font-semibold text-stone-800 border-b border-[var(--line)] lg:pointer-events-none" aria-expanded="false" aria-controls="category-list">
                        Categories
                        <svg class="cat-chevron w-4 h-4 text-stone-400 transition-transform lg:hidden" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 9 6 6 6-6"/></svg>
                    </button>
                    <nav id="category-list" class="hidden lg:block shop-category-scroll lg:overflow-auto lg:h-[67vh] divide-y divide-[var(--line)]">
                            ...
                            <a href="{{ route('shop.catalog', array_filter(['category' => $category->slug, 'q' => request('q')])) }}"
                               class="flex items-center gap-3 px-4 py-3 text-sm transition-colors {{ request('category') === $category->slug ? 'text-brand font-semibold bg-cream' : 'text-stone-600 hover:bg-cream/60' }}">
                                @if($category->imageUrl())
                                    <img src="{{ $category->imageUrl() }}" alt="" class="w-7 h-7 rounded-full object-cover shrink-0">
                                @endif
